...
public class AestheticApiTest
{
    ...
    #region Action Tests: Api Version - V2 contains SQLite database

    [Fact]
    public async Task GetAestheticV2r̥_ReturnsOkResult_WhenAestheticExist()
    {
        var aesthics = new List<AestheticDto>
            {
                new AestheticDto {  Name = "Brewery 1", City = "City 1", Phone = "123456" },
    ...
    [Fact]
    public async Task GetAestheticV2r̥_ReturnsNotFound_WhenNoAestheticExist()
    {
        _mockAestheticSvc.Setup(s => s.GetAestheticsAsync_DB(null, null, null, null)).ReturnsAsync(new List<AestheticDto>());

        var result = await _aestheticController.GetAestheticsAsync_Db(null, null, null, null);

        Assert.IsType<NotFoundResult>(result.Result);
    }

    [Fact]
    public async Task AutocompleteV2r̥_ReturnsOkResult_WhenAestheticExist()
    {
        var aesthics = new List<string> { "Brewery 1", "City 1", "Brewery 2", "City 2" };
        _mockAestheticSvc.Setup(s => s.Autocomplete_DB(null)).ReturnsAsync(aesthics);

        var result = await _aestheticController.AutocompleteAsnc_Db(null);
    ...
    [Fact]
    public async Task AutocompleteV2r̥_ReturnsNotFound_WhenNoAestheticExist()
    {
        _mockAestheticSvc.Setup(s => s.Autocomplete_DB(null)).ReturnsAsync(new List<string>());
